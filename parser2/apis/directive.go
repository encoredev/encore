...
func (p *Parser) parseDirectives(cg *ast.CommentGroup) (d directive, doc string) {
	...
	for _, c := range cg.List {
		...
		if strings.HasPrefix(c.Text, prefix) {
			if dir != nil {
				p.c.Errs.Add(c.Pos(), "cannot have multiple encore annotations")
				continue
			}
			var err error
			dir, err = parseDirective(c.Pos(), c.Text[len(prefix):])
			if err != nil {
				p.c.Errs.Add(c.Pos(), err.Error())
			}
			err = validateDirective(dir)
			if err != nil {
				p.c.Errs.Add(cg.Pos(), err.Error())
			}
		}
	}
	if dir != nil {
	...
	for _, line := range lines {
		...
		if strings.HasPrefix(line, prefix) {
			if dir != nil {
				p.c.Errs.Add(cg.Pos(), "cannot have multiple encore annotations")
				continue
			}
			var err error
		...
		for _, field := range fields[1:] {
			...
			default:
				if strings.HasPrefix(field, "tag:") {
					...
					case "path":
						var err error
						rpc.Path, err = apipaths.Parse(pos, parts[1])
						if err != nil {
...
type rpcDirective struct {
	...
	Raw      bool
	Method   []string
	Path     *apipaths.Path // nil if not specified
	Tags     selector.Set
}
