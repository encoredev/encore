...
await assertStructuredError(api.test.NoopWithError(), ErrCode.Unimplemented, "totally not implemented yet")
...
deepEqual(restRsp.HeaderValue, "this is the header field", "expected header value")
deepEqual(restRsp.QueryValue, "this is a query string field", "expected query value")
deepEqual(restRsp["Some-Key"], "this is the body field", "expected body value")
deepEqual(restRsp.Nested.Alice, "hello + the nested key", "expected nested key")
...
const params: MarshallerTest<number> = {
  ...
  HeaderTime:    (new Date(Math.floor(Date.now() / 1000) * 1000)).toISOString().replace(".000Z", "Z"),
  ...
  QueryBytes:    "d29ybGQK",
  QueryTime:     (new Date(Math.floor(Date.now() / 1000) * 1000)).toISOString().replace(".000Z", "Z"),
  ...
  time:          (new Date(Math.floor(Date.now() / 1000) * 1000)).toISOString().replace(".000Z", "Z"),
...
async function assertStructuredError(promise: Promise<any>, code: ErrCode, message: string) {
  ...
  try {
    ...
    if (isAPIError(err)) {
      if (err.code !== code) {
        throw new Error(`Expected error code ${code}, got ${err.code} with message "${err.message}"`)
      }
      if (err.message !== message) {
        throw new Error(`Expected error message "${message}", got "${err.message}"`)
      }
    } else {
