...
To create a SQL database using Encore we first create a folder named `migrations` and inside that folder a migration file named 1*create_tables.up.sql. The file name is important (it must look something like 1*&lt;name&gt;.up.sql). Our migration file is only five lines long and looks like this:
