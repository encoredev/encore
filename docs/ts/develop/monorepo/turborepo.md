...
The `prebuild` hook runs when deploying via Encore Cloud or when exporting a Docker image with the Encore CLI. The filter `@repo/backend...` tells Turborepo to build `@repo/backend` and all its dependencies, ensuring the shared package is compiled before Encore builds your application.
...
- **Turborepo filter**: Using `--filter=@repo/backend...` builds the backend and all its dependencies
