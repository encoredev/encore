...
Encore.ts has request validation built in, and it's designed to work seamlessly with TypeScript. It uses the natural TypeScript types directly to validate incoming requests, so you get the best of both worlds: the clean, concise TypeScript syntax, and runtime schema validation. This means that your APIs are type-safe during both on both runtime and compile-time.
...
It is often the case that you want to return the same data type that you received in the request. In this case, you can reuse the request type as the response type:
