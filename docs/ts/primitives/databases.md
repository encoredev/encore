...
* Open your app in the [Encore Cloud dashboard](https://app.encore.dev), navigate to the **Infrastructure** page for the appropriate environment, and locate the `USERS` section within the relevant **Database Cluster**.
...
```ts
await db.exec`SET ROLE matview_owner`;
await db.exec`REFRESH MATERIALIZED VIEW my_view`;
