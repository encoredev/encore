...
Encore.ts does not support default values in request or response schemas. While TypeScript itself
does not have syntax for default values on `interface` or `type` fields, you may be tempted to
reach for patterns like assigning defaults inside the handler or relying on a class with property
initializers. These patterns are not applied by Encore's request parser — incoming requests are
validated and decoded strictly against the declared schema, and missing fields remain `undefined`
unless the field is marked optional with `?`.

If you need a default value, mark the field as optional and apply the default inside your handler:
...
export const list = api(
  ...
  async (p: ListParams): Promise<Response> => {
