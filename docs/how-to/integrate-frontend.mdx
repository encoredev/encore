...
### Testing
When unit testing components that are interacting with your Encore API you can mock methods on the request client to
return a value suitable for the test. This will make your tests URL agnostic because you are not intercepting
specific requests on the fetch layer. You will also get type errors in your tests if the request client gets updated.

Here is an example from the [Uptime Monitoring](https://github.com/encoredev/examples/tree/main/uptime) example where we
are mocking a GET request method and spying on a POST request method:
