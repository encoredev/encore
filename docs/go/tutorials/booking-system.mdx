...
With Encore you define a service by [defining one or more APIs](/docs/go/primitives/defining-aps) within a regular Go package. Encore recognizes this as a service, and uses the package name as the service name. When deploying, Encore will automatically [provision the required infrastructure](/docs/platform/infrastructure/infra) for each service.
