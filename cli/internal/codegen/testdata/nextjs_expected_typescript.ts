...
export default class Client {
    public readonly products: ServiceClient_products
    public readonly svc: ServiceClient_svc
    ...
    constructor(target: string | BaseURL = "prod", options?: string | ClientOptions) {
        ...
        this.products = new ServiceClient_products(base)
        this.svc = new ServiceClient_svc(base)
    }
}
...
export interface products_CreateProductRequest {
...
export interface products_Product {
    ...
    description: string
    "created_at": string
    "created_by": authentication_User
}

export interface products_ProductListing {
...
export class ServiceClient_products {
    ...
    public async Create(params: products_CreateProductRequest): Promise<products_Product> {
        ...
        try {
            return await resp.json() as products_Product
        } catch (err) {
            throw new APIError(500, { code: ErrCode.DataLoss, message: "unable to unmarshal the response", details: err })
        }
    }

    public useCreate(params: products_CreateProductRequest, swrConfig?: SWRConfiguration<products_Product, APIError>): SWRResponse<products_Product, APIError> {
        return useSWR(
    ...
    public async List(): Promise<products_ProductListing> {
        // Now make the actual call to the API
        const resp = await this.baseClient.callAPI("GET", `/products.List`)
        try {
            return await resp.json() as products_ProductListing
        } catch (err) {
            throw new APIError(500, { code: ErrCode.DataLoss, message: "unable to unmarshal the response", details: err })
        }
    }

    public useList(swrConfig?: SWRConfiguration<products_ProductListing, APIError>): SWRResponse<products_ProductListing, APIError> {
        return useSWR(
            [`/products.List`],
            () => this.List(),
            swrConfig
...
export interface svc_AllInputTypes<A> {
    /**
...
export type svc_Foo = number

export interface svc_GetRequest {
    Baz: number
}
 ...
 */
export interface svc_HeaderOnlyStruct {
    Boolean: boolean
    Int: number
    Float: number
    String: string
 ...
 */
export interface svc_Tuple<A, B> {
    A: A
    B: B
}

export type svc_WrappedRequest = svc_Wrapper<svc_Request>

export interface svc_Wrapper<T> {
    Value: T
}

export class ServiceClient_svc {
    private baseClient: BaseClient
     ...
     */
    public async DummyAPI(params: svc_Request): Promise<void> {
     ...
     * DummyAPI is a dummy endpoint.
     */
    public useDummyAPI(params: svc_Request, swrConfig?: SWRConfiguration<void, APIError>): SWRResponse<void, APIError> {
        return useSWR(
            [`/svc.DummyAPI`, params],
            () => this.DummyAPI(params),
            swrConfig
        )
    }

    public async Get(params: svc_GetRequest): Promise<void> {
        // Convert our params into the objects we need for the request
    ...
    public useGet(params: svc_GetRequest, swrConfig?: SWRConfiguration<void, APIError>): SWRResponse<void, APIError> {
        return useSWR(
            [`/svc.Get`, params],
            () => this.Get(params),
            swrConfig
    ...
    public async GetRequestWithAllInputTypes(params: svc_AllInputTypes<number>): Promise<svc_HeaderOnlyStruct> {
        // Convert our params into the objects we need for the request
        const headers: Record<string, string> = {
        ...
        try {
            ...
            const rtn = await resp.json() as svc_HeaderOnlyStruct
    ...
    public useGetRequestWithAllInputTypes(params: svc_AllInputTypes<number>, swrConfig?: SWRConfiguration<svc_HeaderOnlyStruct, APIError>): SWRResponse<svc_HeaderOnlyStruct, APIError> {
        return useSWR(
            [`/svc.GetRequestWithAllInputTypes`, params],
            () => this.GetRequestWithAllInputTypes(params),
            swrConfig
        )
    }

    public async HeaderOnlyRequest(params: svc_HeaderOnlyStruct): Promise<void> {
        // Convert our params into the objects we need for the request
        const headers: Record<string, string> = {
    ...
    public useHeaderOnlyRequest(params: svc_HeaderOnlyStruct, swrConfig?: SWRConfiguration<void, APIError>): SWRResponse<void, APIError> {
        return useSWR(
            [`/svc.HeaderOnlyRequest`, params],
            () => this.HeaderOnlyRequest(params),
            swrConfig
        )
    }
    ...
    public useRESTPath(a: string, b: number, swrConfig?: SWRConfiguration<void, APIError>): SWRResponse<void, APIError> {
        return useSWR(
            [`/path/${encodeURIComponent(a)}/${encodeURIComponent(b)}`],
            () => this.RESTPath(a, b),
            swrConfig
        )
    }

    public async RequestWithAllInputTypes(params: svc_AllInputTypes<string>): Promise<svc_AllInputTypes<number>> {
        // Convert our params into the objects we need for the request
        const headers: Record<string, string> = {
            "x-alice": String(params.A),
        }
        ...
        try {
            ...
            //Populate the return object from the JSON body and received headers
            const rtn = await resp.json() as svc_AllInputTypes<number>
    ...
    public useRequestWithAllInputTypes(params: svc_AllInputTypes<string>, swrConfig?: SWRConfiguration<svc_AllInputTypes<number>, APIError>): SWRResponse<svc_AllInputTypes<number>, APIError> {
        return useSWR(
            [`/svc.RequestWithAllInputTypes`, params],
            () => this.RequestWithAllInputTypes(params),
            swrConfig
        )
    }
     ...
     */
    public async TupleInputOutput(params: svc_Tuple<string, svc_WrappedRequest>): Promise<svc_Tuple<boolean, svc_Foo>> {
        // Now make the actual call to the API
        const resp = await this.baseClient.callAPI("POST", `/svc.TupleInputOutput`, JSON.stringify(params))
        try {
            return await resp.json() as svc_Tuple<boolean, svc_Foo>
        } catch (err) {
            throw new APIError(500, { code: ErrCode.DataLoss, message: "unable to unmarshal the response", details: err })
        }
    }
     ...
     * and this comment is also multiline, so multiline comments get tested as well.
     */
    public useTupleInputOutput(params: svc_Tuple<string, svc_WrappedRequest>, swrConfig?: SWRConfiguration<svc_Tuple<boolean, svc_Foo>, APIError>): SWRResponse<svc_Tuple<boolean, svc_Foo>, APIError> {
        return useSWR(
            [`/svc.TupleInputOutput`, params],
            () => this.TupleInputOutput(params),
            swrConfig
        )
    }
    ...
    public useWebhook(method: string, a: string, b: string[], body?: BodyInit, options?: CallParameters, swrConfig?: SWRConfiguration<Response, APIError>): SWRResponse<Response, APIError> {
        return useSWR(
            [`/webhook/${encodeURIComponent(a)}/${b.map(encodeURIComponent).join("/")}`],
            () => this.Webhook(method, a, b, body, options),
            swrConfig
        )
    }
}
