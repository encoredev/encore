...
const RPCCaller: FC<Props> = ({ md, svc, rpc, conn, appID, addr }) => {
  ...
  return (
    <div>
      ...
      <div
        className={`border-gray-200 mt-1 rounded border text-xs ${
      ...
      {response ? (
        <pre className="shadow-inner border-gray-300 bg-gray-200 response-docs overflow-x-auto rounded border p-2 text-xs">
          <CM
            key={response}
            cfg={{
              value: response,
...
const APICallButton: FC<{ send: () => void; copyCurl: () => void }> = (
  ...
  return (
    <span className="shadow-sm relative z-0 ml-auto inline-flex flex-none self-start rounded-md">
      <button
        type="button"
        className="border-purple-700 bg-purple-600 hover:bg-purple-500 focus:outline-none focus:border-purple-500 relative inline-flex items-center rounded-l-md border px-4 py-2 text-sm font-medium text-white focus:z-10 focus:ring-0"
        onClick={() => props.send()}
      >
        Call API
      </button>
      <span className="relative z-10 -ml-px block">
        <Menu>
          {({ open }) => (
            <>
              <Menu.Button className="border-purple-700 bg-purple-600 hover:bg-purple-500 focus:outline-none relative inline-flex items-center rounded-r-md border px-2 py-2 text-sm font-medium text-white focus:z-10 focus:ring-0">
        ...
        <Listbox value={method} onChange={setMethod}>
          {({ open }) => (
            <div className="relative">
              <Listbox.Button className="focus:outline-none text-green-800 bg-green-100 hover:bg-green-200 relative block cursor-default rounded-sm py-0.5 pl-1 pr-5 text-left font-mono text-xs font-semibold">
              ...
              >
                <Listbox.Options
                  static
                  className="shadow-lg focus:outline-none absolute z-10 mt-1 max-h-60 w-32 overflow-auto rounded bg-white py-1 text-xs ring-1 ring-black ring-opacity-5"
                >
