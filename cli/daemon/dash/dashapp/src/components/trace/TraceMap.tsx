...
const TraceMap: FunctionComponent<Props> = (props) => {
  ...
  const renderSpan = (req: Request, line: number) => {
    ...
    return <React.Fragment key={req.id}>
      <style>{`
        .mapspan-${req.id}       { background-color: ${sel ? highlightColor : color}; }
        .mapspan-${req.id}:hover { background-color: ${highlightColor}; }
      `}</style>
      <div key={req.id} className={`absolute mapspan-${req.id}`} onClick={select}
          style={{
            borderRadius: "3px",
  ...
  return (
    <div className="relative" style={{height: (lines.length * (lineHeight+lineGap) - lineGap) + "px"}}>
      {lines.map((line, i) =>
        line.map((span, j) =>
          renderSpan(span, i)
      ))}
