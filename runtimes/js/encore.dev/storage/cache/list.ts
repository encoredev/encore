 ...
 * @internal
 */
abstract class ListKeyspaceBase<K, V> extends KeyspaceBase<K> {
   ...
   */
  async setIndex(key: K, index: number, value: V, options?: WriteOptions): Promise<void> {
    const source = getCurrentRequest();
    const mappedKey = this.mapKey(key);
   ...
   * @param stop - Stop index (inclusive)
   */
  async trim(key: K, start: number, stop: number, options?: WriteOptions): Promise<void> {
    const source = getCurrentRequest();
    const mappedKey = this.mapKey(key);
    const ttlMs = this.resolveTtl(options);
   ...
   */
  async insertBefore(key: K, pivot: V, value: V, options?: WriteOptions): Promise<number> {
    const source = getCurrentRequest();
    const mappedKey = this.mapKey(key);
   ...
   * @returns The length of the list after the operation, or -1 if pivot not found.
   */
  async insertAfter(key: K, pivot: V, value: V, options?: WriteOptions): Promise<number> {
    const source = getCurrentRequest();
    const mappedKey = this.mapKey(key);
    const pivotSerialized = this.serializeItem(pivot);
    const valueSerialized = this.serializeItem(value);
  ...
  async removeAll(key: K, value: V, options?: WriteOptions): Promise<number> {
    ...
    const valueSerialized = this.serializeItem(value);
    const ttlMs = this.resolveTtl(options);
    const result = await this.cluster.impl.lrem(mappedKey, 0, valueSerialized, ttlMs, source);
    return Number(result);
  }
   ...
   * @returns The number of elements removed.
   */
  async removeFirst(key: K, count: number, value: V, options?: WriteOptions): Promise<number> {
    if (count < 0) {
    ...
    const valueSerialized = this.serializeItem(value);
    const ttlMs = this.resolveTtl(options);
    const result = await this.cluster.impl.lrem(mappedKey, count, valueSerialized, ttlMs, source);
    return Number(result);
  }
   ...
   * @returns The number of elements removed.
   */
  async removeLast(key: K, count: number, value: V, options?: WriteOptions): Promise<number> {
    if (count < 0) {
      throw new Error("count must be non-negative");
    }
    const source = getCurrentRequest();
   ...
   */
  async move(src: K, dst: K, srcPos: ListPosition, dstPos: ListPosition, options?: WriteOptions): Promise<V | undefined> {
    const source = getCurrentRequest();
 ...
 * ```
 */
export class StringListKeyspace<K> extends ListKeyspaceBase<K, string> {
  constructor(cluster: CacheCluster, config: KeyspaceConfig<K>) {
    super(cluster, config);
  }
 ...
 * ```
 */
export class NumberListKeyspace<K> extends ListKeyspaceBase<K, number> {
  constructor(cluster: CacheCluster, config: KeyspaceConfig<K>) {
    super(cluster, config);
  }
