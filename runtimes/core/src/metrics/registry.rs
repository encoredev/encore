...
/// This allows different runtime implementations to provide their metrics without
/// creating a direct dependency from core to the specific runtime.
...
impl std::fmt::Debug for Registry {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("Registry")
            ...
            .field("external_collectors", &"<collectors>")
            .finish()
    }
}
...
impl Registry {
    ...
    pub fn register_collector(&self, collector: Arc<dyn MetricsCollector>) {
        self.external_collectors.write().unwrap().push(collector);
    }
    ...
    pub fn collect(self: &Arc<Self>) -> Vec<CollectedMetric> {
        ...
        let collectors = self.external_collectors.read().unwrap();
